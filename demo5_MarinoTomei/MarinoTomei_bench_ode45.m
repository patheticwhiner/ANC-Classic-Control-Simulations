...
for i = 1:num_steps
    ...
    % 使用ode45求解
    [~, states] = ode45(@(t, states) system_dynamics(t, states, a1, b2, beta, u, D, g, r_omega, epsilon_r, R, P1, P2, P3, q1, q2, q3), tspan_local, all_states);
...
%% 系统动态方程定义
function dstates = system_dynamics(t, states, a1, b2, beta, u, D, g, r_omega, epsilon_r, R, P1, P2, P3, q1, q2, q3)
    % 提取状态变量
    plant_x = states(1:2);
    xi1 = states(3:6);
    xi2 = states(7:10);
    ...
    % 估计器χ动态方程
    dchi_hatdt = D*chi_hat - [D(1,1); D(2,1); D(3,1); D(4,1)]*u;
    ...
    % 组合所有导数
    dstates = [dx1dt; dx2dt; dxi1dt; dxi2dt; dchi_hatdt; theta1_hat_dot; theta2_hat_dot; ddist_x];
end
