...
| 阶段 | 论文年份 | 核心问题 | 对应脚本 |
|:---|:---|:---|:---|
| **阶段2** | 2011 | 被控对象不完全已知，外系统频率已知但参数未知 | `MarinoTomei_ode45`, `_euler`, `_bench_*`, `demo_simple_adaptive` |
...
- **结构分析**: `MarinoTomei_internal_model`
...
| 脚本 | 求解器 | 额外功能 | 运行速度 |
|:---|:---|:---|:---|
| `MarinoTomei_ode45.m` | ode45 (变步长) | 参考控制器 $R_c$ | ~2s |
| `MarinoTomei_euler.m` | Euler (dt=0.01) | 参考控制器 $R_c$ | ~1s |
| `MarinoTomei_bench_ode45.m` | ode45 | + 调节器方程基准 | ~10s |
| `MarinoTomei_bench_rk4.m` | RK4 (dt=0.01) | + 调节器方程基准 | ~5s |
...
1. **首选 ode45**: 变步长自适应, 精度和速度兼顾, `MarinoTomei_ode45.m` 是最佳起点
2. **定步长用于嵌入式**: `MarinoTomei_euler.m` 和 `bench_rk4.m` 展示固定步长可行性
...
| `MarinoTomei_ode45.m` | 265 | 自适应输出调节 | 2011 |
| `MarinoTomei_euler.m` | 272 | 自适应输出调节 (Euler) | 2011 |
| `MarinoTomei_bench_ode45.m` | ~350 | 调节 + 基准 (ode45) | 2011 |
| `MarinoTomei_bench_rk4.m` | ~350 | 调节 + 基准 (RK4) | 2011 |
