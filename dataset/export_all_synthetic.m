...
%% Model 8: Ho 2020 — 窄带 ALE 前馈 ANC (7阶 P + 4阶 S, fs=4kHz)
% Primary path:   P(z) = z^{-5} - 0.3z^{-6} + 0.2z^{-7}
% Secondary path: S(z) = z^{-2} + 1.5z^{-3} - z^{-4}
fs_h = 4000;  Ts_h = 1/fs_h;  z = tf('z', Ts_h);
P = z^(-5) - 0.3*z^(-6) + 0.2*z^(-7);
S = z^(-2) + 1.5*z^(-3) - z^(-4);
model = struct('name','Ho2020_ALE_Narrowband','type','syn_tf_feedforward', ...
    'domain','discrete','P',P,'S',S, ...
    'P_desc','Primary path: P(z)=z^{-5}-0.3z^{-6}+0.2z^{-7}', ...
    'S_desc','Secondary path: S(z)=z^{-2}+1.5z^{-3}-z^{-4} (NMP)', ...
    'Ts',Ts_h,'fs',fs_h,'orders',[7,4], ...
    'source','Ho, C.Y. et al. (2020). Efficient Narrowband Noise Cancellation Using ALE. IEEE/ACM TASLP, 28.', ...
    'desc','Feedforward narrowband ANC with Adaptive Line Enhancer. P: 7th-order FIR primary path. S: 4th-order FIR secondary path with NMP zeros. fs=4kHz.');
save(fullfile(script_dir, 'syn_Ho2020_ALE.mat'), 'model');
fprintf('  [8/8] syn_Ho2020_ALE.mat\n');
