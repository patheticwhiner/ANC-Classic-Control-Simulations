...
|:---|---|
| 域 | 离散时间 |
| 阶数 | 分母 3 阶（3 极点），分子 2 阶（2 零点） |
| 采样周期 | $T_s = 1/480$ s ($f_s = 480$ Hz) |
| 边界零点 | 1 个 ($z=1$)；严格 NMP 零点（$|z|>1$）为 0 个 |
...
- **边界零点 $z=1$** 使 DC 增益严格为零；它阻止 DC 稳定求逆，但不属于单位圆外 NMP 零点
...
- 极点接近单位圆（当前系数计算 $\max|p|\approx0.953$）
...
| 采样频率 | $f_s = 4000$ Hz |
| S(z) 零点 | $z=0.5,-2$；其中 $z=-2$ 为 NMP 零点 |
| 架构 | 前馈窄带 ANC + ALE |
...
1. 合成模型在 `dataset/export_all_synthetic.m` 中保留可复现定义；实测辨识模型保留采集来源和同名验证报告。
2. 生成 `.mat` 文件，并在 `dataset/model_registry.m` 登记来源、角色、表示形式、采样率、证据和使用方。
3. 需要 `DataManager` 加载时填写非空的 `loader_id` 和 `loader_type`，无需再修改 `DataManager.m`。
4. 更新本文档中的公式级说明。
5. 运行 `generate_model_report()`，确认没有新增的缺失资产、未登记文件或内嵌模型候选。
