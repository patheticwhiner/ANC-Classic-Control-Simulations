...
> ⚠️ **约束 C8**：$G_0(z)$ 在扰动频率处的增益决定了**参数尺度的下界**——不是可调的设计选择。$|G_0(e^{j\omega_0})| = 2.3\times10^{-4}$ 迫使 $\|\theta\| \sim 10^4$ 量级以满足插值约束 $G_0K = 1$。这带来三个连锁反应：(i) $\theta_{\max}$ 必须设为 $\gg 10^4$（此处 $10^7$），否则投影会截断收敛中的参数；(ii) $P(0)$ 必须匹配参数尺度（此处 $10^6 I$），否则初始 RLS 步长太小、收敛极慢；(iii) $\theta$ 的大数值范围放大了有限精度误差——$\theta_i$ 之间的加减涉及 $O(10^7)$ 和 $O(10^0)$ 量级的数，在单精度浮点下有效数字丢失严重。若 $G_0$ 的扰动频率增益更小（如 −100 dB），所需的 $\theta$ 量级进一步膨胀，可能超出可用数值精度。
...
> ⚠️ **约束 C9**：H∞ 最优（$\gamma$ 最小化）与**实际降噪性能**并非等价目标。H∞ 范数 $\|G_0K\|_\infty$ 衡量的是最坏情况频率处的增益上界——而最小化它可能以极端的 $\|\theta\|$ 为代价。这些大参数在非扰动频率处将测量噪声放大（通过 $K$ 的带外增益），导致闭环 RMS 噪声不降反升。LS 最小范数解隐式地正则化了 $\|\theta\|$，在实践中更优。这一断裂的原因是：**H∞ 优化不区分扰动能量集中频段和噪声分布频段**——它同等对待所有频率。若你的噪声在扰动频段之外有显著能量，必须在优化中显式加入 $\|\theta\|$ 约束或频率加权。
...
> ⚠️ **约束 C10**：固定控制器的有效带宽由 FIR 阶数 $N$ 和插值约束的「刚性」共同决定。$N$ 越大，插值约束消耗的 2 个自由度在 $N$ 中的占比越小，剩余自由度用于在全频带保持低增益 → 频率偏移容忍度更高。但即使 $N=50$，偏离设计频率 ±30% 后抑制仍显著下降。**这意味着固定方案只有在频率稳定已知的场景（如工频 50 Hz 及其谐波）才适用。** 在汽车发动机噪声控制（频率随 RPM 连续变化）或 HVAC 系统（风机转速可调）中，固定控制器必然失效——自适应不是"更好"，而是"唯一可行的选项"。
...
> ⚠️ **约束 C11**：RLS 更新**必须使用先验误差**（$\theta(k-1)$ 而非 $\theta(k)$），这不是精度问题而是**因果性约束**。若使用后验误差（基于刚更新的 $\theta(k)$ 计算 $\varepsilon$），则在 IMC 结构下形成隐式代数环：$\theta(k)$ 依赖 $z(k)$ → $z(k)$ 依赖 $u(k)$ → $u(k)$ 依赖 $\theta(k)$。在仿真中这表现为一步延迟的数值不稳定性；在实时系统中，如果 DSP 循环严格按时钟驱动，后验误差的计算在物理上不可实现——你需要 $\theta(k)$ 才能算 $u(k)$，但 $\theta(k)$ 的 RLS 更新又需要 $z(k)$（而 $z(k)$ 需要当前 $u(k)$ 通过 $G_0$ 后的输出）。正确的因果顺序是：测量 $y(k)$ → 计算 $z(k)$ → 构造 $\phi(k)$ → 用 $\theta(k-1)$ 算先验误差 → 更新得 $\theta(k)$ → 用 $\theta(k)$ 算 $u(k)$ → 施加控制。
