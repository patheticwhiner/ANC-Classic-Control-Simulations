...
> ⚠️ **约束 C1**：推论 A.1 的慢自适应近似同时要求两个条件——(i) 参数变化远慢于 $H(s)$ 的动态，且 (ii) **$H(s)$ 本身稳定**（冲激响应指数衰减）。若 $H(s)$ 有不稳定极点，即使 $\dot{\theta}$ 任意小，交换误差 $\varepsilon_H(t)$ 也不再有界，线性参数化 (A.7) 不再成立。在题1/题2中，这意味着 $G_0(s)$ 和 $F(s)$ 必须都是稳定的——$F(s)$ 的设计（RHP 零点反射后替换为 LHP 零点）正是为了保证这一点。
...
**注释 A.2**：归一化是自适应控制鲁棒化的关键步骤。它保证了即使 $\phi(t)$ 无界（如系统不稳定），$\varepsilon(t)$ 仍然有界。

> ⚠️ **约束 C2**：归一化不是可选的增强特性，而是**防止参数漂移的必要条件**。若无 $m_s^2(t)$（即 $\varepsilon = e$ 而不除以归一化因子），则 $\dot{\theta} = P e \phi$ 中的更新量直接正比于 $\|\phi\|$——在瞬态过程中 $\phi$ 可能因初始误差或未建模动态激励而大幅增长，自适应律随之发散。式 (A.9) 中的常数项 "1" 同样不可省略——它防止 $\phi = 0$ 时除以零。在实际代码中，若忘记归一化或错误地使用 $m_s^2 = \phi^T\phi$（无 +1），在信号静默期会出现除零或数值爆炸。
...
> ⚠️ **约束 C3**：投影算子预设 $\theta^* \in \Theta$——即**真实参数必须在可行域内**。若 $\theta_{\max}$ 选得太小而排除了真值（例如在题2中 $\|\theta_{LS}\| \approx 7\times10^4$ 而误设 $\theta_{\max}=100$），投影不再是保护而是**破坏**——它会将参数截断在远离真值的位置，自适应永远无法收敛。$\theta_{\max}$ 的选择必须同时满足两个条件：(i) $\theta_{\max} \ge \|\theta^*\|$（涵盖真值），且 (ii) 满足小增益条件 (A.17)（保证鲁棒稳定性）。这两个条件的方向相反——前者要求 $\theta_{\max}$ 足够大，后者要求它足够小——存在可行解本身就是一个需要验证的前提。
...
> ⚠️ **约束 C4**：小增益条件 (A.17) 是**充分条件而非必要条件**。即使在某些频率处环路增益超过 1，若相位不满足振荡条件，系统仍可能稳定。但反过来——若 (A.17) 不成立，你**不能**声称系统一定会失稳，只能说你失去了鲁棒稳定性的**数学保证**。在实践中，这意味着若 LHS = 1.2 > 1，系统可能仍然稳定，但你需要额外的分析（如 Nyquist 轨迹检查）来确认。文档中所有声称「留有 X% 裕度」的数值都基于这个充分性假设。
...
> ⚠️ **约束 C6**：`hinfsyn` 内部的回路变换虽然自动满足 DGKF 的简化假设，但它会**改变权重函数的有效形状**。你通过 `augw` 传入的 $W_1(s), W_3(s)$ 和求解器实际使用的可能不同——尤其是在 $D_{12}$ 或 $D_{21}$ 不满列/行秩时，回路变换引入的缩放因子会等效地修改权重的 DC 增益和带宽。因此，`hinfsyn` 返回 $\gamma < 1$ 后**仍然必须**通过 Bode 图验证 $|S| < |1/W_1|$ 和 $|T| < |1/W_3|$ 在所有频率成立——不能仅靠 $\gamma$ 值判断设计成功。
...
> ⚠️ **约束 C5**：耦合条件 $\rho(X_\infty Y_\infty) < \gamma^2$ 定义了 H∞ 设计的**物理性能上限**。无论权重如何设置，$\gamma$ 不可能低于由被控对象物理特性（RHP 极点、RHP 零点、延迟等）决定的下界 $\gamma_{\text{opt}}$。对于非最小相位系统，$\gamma_{\text{opt}}$ 严格大于某个正数——这意味着存在一些性能要求是**物理上不可实现的**，无论你如何设计控制器。当 `hinfsyn` 二分搜索失败（$\gamma \to \infty$ 或返回 NaN）时，根本原因通常是权重要求超过了这个物理极限，而非算法或代码问题。
